...
class UNumericKeyboard extends StatelessWidget {
  ...
  final UNumericKeyboardAction? leadingKey;
  final UNumericKeyboardAction? trailingKey;
  ...
  Widget _grid(BuildContext context, double keyHeight) => Column(
    children: <Widget>[
      ...
      SizedBox(
        ...
        child: Row(
          children: <Widget>[
            Expanded(child: leadingKey == null ? const SizedBox.shrink() : _actionKey(context, leadingKey!)),
            SizedBox(width: spacing),
            Expanded(child: _digitKey(context, "0")),
            SizedBox(width: spacing),
            Expanded(child: _trailing(context)),
          ],
        ),
      ),
    ],
  ...
  Widget _trailing(BuildContext context) {
    if (trailingKey != null) return _actionKey(context, trailingKey!);
    if (onBackspace == null) return const SizedBox.shrink();
    return _actionKey(
      context,
